...
var Guitar = function(options) {
    ...
    var getFretboard = function(){
    	...
    	// loop em
    	_.each(strings, function(string, index){ fretboard[index+1] = getStringNotes(index+1); });
    ...
    var getStringNotes = function(string, frets){
    	...
    	var scale = new Scale({key:open}).getChromaticScale();

    	// concat scale to number of octaves
    	var notes = _.flatten(_.times(octaves, function(){ return scale; }));
